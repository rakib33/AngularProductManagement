...
export class TableMainDirective implements OnInit, OnDestroy {
    ...
    sortingRunMode: number = 0; //0: single column sorting only, 1: single/multiple column sorting.
    sortingTypeSwitch: number = 0; //0: dropdown selection mode, or 1: Ctrl/Shift key mode - used only for sortingRunMode = 1.
    sortingOption: string = 'multiple'; //'single' or 'multiple' - initial loading type used only for sortingRunMode = 1.    
    ...
    cancelMultiSortAction() {
        ...
        if (this.sortingTypeSwitch == 1 && !this.baseSequenceOptions.find(a => a.sequence > 1)) {
            this.sortingOption = 'single';
        }
    }
